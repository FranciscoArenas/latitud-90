              ...
              <div class="bg-blue-50 p-6 rounded-lg">
                <div class="flex items-center">
                  ...
                  <div>
                    ...
                    <p class="text-2xl font-bold text-blue-600">${{ stats.total_revenue.toLocaleString() }}</p>
                  </div>
                </div>
              </div>
              ...
              <div class="bg-purple-50 p-6 rounded-lg">
                <div class="flex items-center">
                  ...
                  <div>
                    ...
                    <p class="text-2xl font-bold text-purple-600">{{ stats.refunded }}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr v-for="payment in filteredPayments" :key="payment.id" class="hover:bg-gray-50">
                      ...
                      <div class="flex items-center">
                        <div class="h-8 w-8 bg-gray-200 rounded-full flex items-center justify-center mr-3">
                          <span class="text-xs font-medium text-gray-700">
                            {{ (payment.passenger.first_name + ' ' + payment.passenger.last_name).charAt(0).toUpperCase() }}
                          </span>
                        </div>
                        <div>
                          <div class="text-sm font-medium text-gray-900">{{ payment.passenger.first_name }} {{ payment.passenger.last_name }}</div>
                          <div class="text-sm text-gray-500">{{ payment.passenger.email }}</div>
                        </div>
                      </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                      <div class="flex items-center">
                        <span :class="getGatewayClass(payment.payment_method)" class="px-2 py-1 text-xs font-semibold rounded-full">
          ...
          <div v-if="selectedPayment" class="space-y-4">
            <div class="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p class="mt-1 text-sm text-gray-900">{{ selectedPayment.transaction_id }}</p>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700">Pasajero</label>
                <p class="mt-1 text-sm text-gray-900">{{ selectedPayment.passenger.name }}</p>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700">Programa</label>
                <p class="mt-1 text-sm text-gray-900">{{ selectedPayment.program.title }}</p>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700">Método de Pago</label>
                <span :class="getGatewayClass(selectedPayment.payment_gateway)" class="mt-1 px-2 py-1 text-xs font-semibold rounded-full">
                  {{ selectedPayment.payment_gateway.toUpperCase() }}
                </span>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700">Estado</label>
                <span :class="getStatusClass(selectedPayment.status)" class="mt-1 px-2 inline-flex text-xs leading-5 font-semibold rounded-full">
                  {{ getStatusText(selectedPayment.status) }}
                </span>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700">Monto</label>
                <p class="mt-1 text-sm text-gray-900">${{ selectedPayment.amount.toLocaleString() }}</p>
              </div>
              <div>
            ...
            <div v-if="selectedPayment.gateway_response">
              <label class="block text-sm font-medium text-gray-700">Respuesta de la Pasarela</label>
              <pre class="mt-1 text-xs text-gray-900 bg-gray-100 p-2 rounded">{{ JSON.stringify(selectedPayment.gateway_response, null, 2) }}</pre>
            </div>
          </div>
        </div>
      </div>
...
const filteredPayments = computed(() => {
  ...
  if (filterGateway.value) {
    filtered = filtered.filter(payment => payment.payment_gateway === filterGateway.value)
  }
